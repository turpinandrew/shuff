...
*/
inline void OUTPUT_ULONG_DEBUG(FILE* f, uint32_t n, char len)
{
    fprintf(stderr, "n=%" PRIu32 " len=%d *buff=%" PRIu32 " btg=%" PRId32 " ",
        n, len, *buff, buff_btg);
    if (len < buff_btg) {
        *buff <<= len;
        *buff |= n;
        buff_btg -= len;
        fprintf(
            stderr, "NOW *buff=%" PRIu32 " btg=%" PRId32 "\n", *buff, buff_btg);
    } else {
        *buff <<= buff_btg;
        *buff |= (n) >> (len - buff_btg);
        OUTPUT_NEXT(f);
        *buff = n;
        ...
        fprintf(stderr,
            "NOW *(buff-1)=%" PRIu32 "  *(buff)=%" PRIu32 "  btg=%" PRId32 "\n",
            *(buff - 1), *buff, buff_btg);
    }
}

inline void OUTPUT_ULONG(FILE* f, uint32_t n, char len)
{
    if (len < buff_btg) {
        *buff <<= len;
        *buff |= n;
...
//
inline void OUTPUT_UNARY_CODE(FILE* f, int32_t n)
{
    for (; n > 0; n--)
        OUTPUT_BIT(f, 0);
    OUTPUT_BIT(f, 1);
...
**
*/
inline void FINISH_OUTPUT(FILE* f)
{
...
inline int32_t START_INPUT(FILE* f)
{
...
//
inline uint32_t INPUT_ULONG(FILE* f, int32_t len)
{
    if (len == 0)
        return 0;
...
inline uint32_t INPUT_ULONG_DEBUG(FILE* f, int32_t len)
{
    if (len == 0)
        return 0;
    ...
    fprintf(stderr,
        "n=%" PRIu32 " len=%" PRId32 " *buff=%" PRIu32 " btg=%" PRId32 " ", n,
        len, *buff, buff_btg);
    if (buff_btg == BUFF_BITS)
        n = (*buff) >> (BUFF_BITS - len);
    else
...
inline uint32_t INPUT_BIT(FILE* f)
{
    buff_btg--;
...
//
inline uint32_t INPUT_UNARY_CODE(FILE* f)
{
    uint32_t n;
    //    uint32_t b = ((*buff) << (BUFF_BITS-buff_btg)) >> (BUFF_BITS -
    //    buff_btg);
    //
