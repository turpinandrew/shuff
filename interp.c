...
*/
void interp_encode(FILE* out_file, uint32_t A[], uint32_t n)
{
    ...
    while (STACK_NOT_EMPTY) {
        ...
        if ((hi - mid > 1) && (A[hi] - A[mid] > (uint32_t)(hi - mid)))
            PUSH(mid, hi);
        if ((mid - lo > 1) && (A[mid] - A[lo] > (uint32_t)(mid - lo)))
            PUSH(lo, mid);
    }
} /* interp_encode() */
...
*/
void interp_decode(FILE* in_file, uint32_t A[], uint32_t n)
{
